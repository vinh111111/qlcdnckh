                ...
                <div class="col-sm-12">
                    <div class="iq-card">
                        ...
                        <div class="iq-card-body">
                            ...
                            <form action="{{route('admin.postApproveOrCancelProject', $project->id)}}" method="post" enctype="multipart/form-data">
                                @csrf
                                @method('put') 
                                @if($project->report_link != '')
                                    <div class="form-group">
                                        <label>Đường dẩn mới báo cáo:</label>
                                        <input type="url" class="form-control" name="new_report_link">
                                    </div>
                                @endif
                                @if($project->product_link != '')
                                    <div class="form-group">
                                        <label>Đường dẩn mới đến sản phẩm:</label>
                                        <input type="url" class="form-control" name="new_product_link">
                                    </div> 
                                @endif
                                @if($project->application_link != '')
                                    <div class="form-group">
                                        <label>Đường dẩn mới đến ứng dụng:</label>
                                        <input type="url" class="form-control" name="new_application_link">
                                    </div>
                                @endif
                                <div class="form-group">
                                    <label>Đánh giá:</label>
                                ...
                                <button type="submit" class="btn btn-primary" name="action" value="approve">Duyệt đề án</button>
                                <button type="submit" class="btn btn-secondary" name="action" value="cancel">Hủy đề án</button>
                                <a href="{{ route('admin.getApproveProjectList') }}" class="btn btn-danger"><font color="white">Trở lại</font></a>
                            </form>
                        </div>
                     </div>
                  </div>
